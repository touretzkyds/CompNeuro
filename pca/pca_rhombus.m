...
if ~exist('demo_type','var'), demo_type = 'pca_rhombus'; end
if ~strcmp(demo_type,'pca_rhombus'), clear all; demo_type = 'pca_rhombus'; end
if ~exist('Npts','var'), Npts = 3000; end
if ~exist('Nepochs','var'), Nepochs = 250; end

if ~exist('eta','var'), eta = 0.8; end
...
plot([-50 50],[0 0],'w--')
plot([0 0],[-50 50],'w--')
plot(rpts(1,:),rpts(2,:),'y.','MarkerSize',4)
hw1 = []; hw2 = []; hm1 = []; hm2 = []; hp = [];
